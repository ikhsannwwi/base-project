...
Parsley.on("field:validated", function (el) {
  ...
  if (elNode && !elNode.isValid()) {
    var validationResult = Array.isArray(elNode.validationResult) ? elNode.validationResult : [elNode.validationResult];
    var rqeuiredValResult = validationResult.filter(function(vr) {
        return vr.assert && vr.assert.name === "required";
    });
    if (rqeuiredValResult.length > 0) {
      ...
      if (lblNode.length > 0) {
        ...
        if (errorNode.length > 0) {
          var lblText = lblNode.data('parsley-column') ? lblNode.data('parsley-column') : lblNode.text();
          if (lblText) {
